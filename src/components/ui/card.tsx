...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-white text-cascala-gray-900 flex flex-col rounded-lg border-2 border-cascala-gray-200 shadow-card transition-shadow duration-200",
        className
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-cascala-gray-500 text-sm leading-relaxed", className)}
      {...props}
    />
  )
}
